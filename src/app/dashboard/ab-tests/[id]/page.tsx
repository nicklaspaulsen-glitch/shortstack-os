    ...
    <div className="space-y-5 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-4 px-1 py-3 sm:py-4">
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-[0.2em] text-text-muted font-editorial italic mb-1">TEST.NAME</p>
          <h1 className="text-2xl font-display font-bold text-text-primary">{test.name}</h1>
        </div>
      ...
      {test.status !== "completed" && (
        <div className="glass rounded-xl p-4 flex items-end gap-3">
          ...
          <button
            ...
            disabled={adding}
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-brand-accent hover:bg-brand-accent/80 disabled:opacity-50 text-[#020711] text-sm font-semibold transition-colors"
          >
            <Plus size={14} />
            {adding ? "Adding…" : "Add"}
          </button>
