  ...
  return (
    ...
    <div className="flex items-center justify-between gap-4 px-1 py-3 sm:py-4">
      <div className="min-w-0">
        <p className="text-xs uppercase tracking-[0.2em] text-text-muted font-editorial italic mb-1">Recurring revenue magnifier</p>
        <h1 className="text-2xl font-display font-bold text-text-primary">Affiliate Program</h1>
      </div>
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-brand-accent text-[#020711] text-sm font-semibold rounded-full hover:bg-brand-accent/80 transition-colors"
        >
      ...
      {programs.length === 0 ? (
        <div className=" border border-dashed border-border-subtle bg-card/40 p-12 text-center">
          ...
          <button
            onClick={() => setShowCreate(true)}
            className="px-4 py-2 bg-brand-accent text-[#020711] text-sm font-semibold rounded-full"
          >
            Create program
          </button>
        </div>
...
function ProgramCard({
  ...
  return (
    <motion.div
      ...
      className="glass rounded-xl p-5 space-y-3 hover:border-[rgba(212,255,0,0.4)] transition-colors spotlight-card"
      ...
      <div className="w-full max-w-lg  bg-card border border-border-subtle p-6 space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            onClick={submit}
            disabled={submitting}
            className="px-4 py-2 text-sm bg-brand-accent text-[#020711] font-semibold rounded-full disabled:opacity-60"
          >
...
function AffiliatesTab({
  ...
  return (
    <div className="space-y-4">
      ...
      {affiliates.length === 0 ? (
        <div className=" border border-dashed border-border-subtle bg-card/40 p-12 text-center">
          <Users size={32} className="mx-auto text-[rgba(212,255,0,0.6)] mb-3" />
...
function StatusBadge({ status }: { status: AffiliateStatus | ReferralStatus | ProgramStatus | CommissionStatus }) {
  const map: Record<string, string> = {
    ...
    signed_up: "bg-[rgba(212,255,0,0.08)] text-brand-accent",
...
function KanbanColumn({
  ...
  const colorMap: Record<string, string> = {
    ...
    blue: "border-[rgba(212,255,0,0.40)]",
