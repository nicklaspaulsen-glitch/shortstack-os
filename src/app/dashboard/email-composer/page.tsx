            ...
            return (
              <MotionPage>
                                ...
                                className={`text-left p-4 rounded-xl border transition-all ${
                                  sel
                                    ? "border-brand-accent bg-[rgba(212,255,0,0.08)] shadow-lg shadow-[rgba(212,255,0,0.1)]"
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-surface-light border border-border-subtle text-sm focus:outline-none focus:border-[rgba(212,255,0,0.4)] focus:ring-2 focus:ring-[rgba(212,255,0,0.12)] transition-all"
 />
          </div>
          <div>
            <label className="block text-[10px] text-text-muted uppercase tracking-wider mb-1.5 font-semibold">
            ...
            <textarea
              ...
              rows={4}
              className="w-full px-4 py-3 rounded-xl bg-surface-light border border-border-subtle text-sm focus:outline-none focus:border-[rgba(212,255,0,0.4)] focus:ring-2 focus:ring-[rgba(212,255,0,0.12)] transition-all resize-none"
              autoFocus
 />
          </div>
        </div>
    ...
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-4 px-1 py-3 sm:py-4">
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-[0.2em] text-text-muted font-editorial italic mb-1">Campaign Editor</p>
          <h1 className="text-2xl font-display font-bold text-text-primary">Email Composer</h1>
        </div>
...
>
 ...
 />
      ...
      {advancedMode && (<>
      {/* Tabs */}
      <PrismPanel padding="p-1" className="flex gap-1 overflow-x-auto">
        {TABS.map(t => (
...
>
 ...
 />
            ...
            <div className="glass flex items-center gap-1 p-1.5 rounded-lg">
              ...
              <div className="w-px h-4 bg-border-subtle mx-1" />
              <button onClick={() => setShowHtml(!showHtml)} className={`p-2 rounded transition-colors flex items-center gap-1 ${showHtml ? "text-indigo-400 bg-indigo-500/10" : "text-text-muted hover:text-text-primary hover:bg-white/6"}`}>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {VARIABLES.map(v => (
                    <button key={v.tag} onClick={() => setEmail(prev => ({ ...prev, body: prev.body + " " + v.tag }))}
                      className="text-left p-2 rounded bg-white/5 hover:border-indigo-500/20 transition-all border border-border-subtle">
              ...
              <div className="rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.88)", border: "1px solid rgba(255,255,255,0.70)" }}>
                ...
                {TEMPLATE_GALLERY.slice(0, 6).map((t, i) => (
                  <motion.button
                    ...
                    className="w-full text-left p-2 text-[10px] transition-all border-b border-border-subtle last:border-0"
>
            ...
            {["all", "Outreach", "Value", "Sales", "Onboarding", "Client", "Billing", "Retention", "Promo"].map(c => (
              <button key={c} onClick={() => setTemplateCategory(c)}
                className={`text-[10px] px-3 py-1.5 rounded-lg ${
                  templateCategory === c ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20" : "text-text-muted border border-border-subtle"
                }`}>{c}</button>
            ))}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-2">
            ...
            {filteredTemplates.map((t, i) => (
              <motion.button
                ...
                className="text-left p-3 rounded-xl hover:border-brand-accent/30 transition-all group glass spotlight-card"
>
          ...
          <div className="flex items-center justify-center gap-2">
            <button onClick={() => setPreviewMode("desktop")}
              className={`text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 ${
                previewMode === "desktop" ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20" : "text-text-muted border border-border-subtle"
        ...
        <div className="space-y-2">
          ...
          {!loadingVariants && subjectVariants.map((v, i) => (
            <button key={i} onClick={() => { setEmail({ ...email, subject: v.subject }); setShowSubjectVariants(false); toast.success("Subject applied"); }}
              className="block w-full text-left p-3 rounded-lg bg-surface-light border border-border-subtle hover:border-[rgba(212,255,0,0.25)] transition-all">
