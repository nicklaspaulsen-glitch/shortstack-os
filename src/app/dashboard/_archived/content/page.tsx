...
export default function ContentPage() {
  ...
  return (
    <MotionPage className="space-y-6">
      ...
      {/* ── Drop & Go ───────────────────────────────────────────── */}
      <div className="glass-indigo rounded-xl p-4 border border-[rgba(212, 255, 0,0.25)] bg-gradient-to-br from-[rgba(212, 255, 0,0.05)] to-transparent">
            ...
            {planThemes.length > 0 && (
              <div className="mb-3 flex flex-wrap gap-1.5">
                {planThemes.slice(0, 12).map((t) => (
                  <span key={t.week} className="text-[10px] px-2 py-1 rounded border border-[rgba(212, 255, 0,0.2)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent">
                    W{t.week}: {t.theme}
                  </span>
                ))}
            ...
            <div className="space-y-2">
              ...
              {planPeriodDays < 30 && (
                <button
                  ...
                  className="w-full text-left p-3 rounded-lg border border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] hover:bg-[rgba(212, 255, 0,0.08)] transition flex items-start gap-3"
                >
                      ...
                      <div key={idea.id} className="card-hover p-4">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-1">
                            {!idea.is_approved && (
                              <button onClick={() => approveIdea(idea.id)} className="text-xs text-brand-accent hover:text-[#D4FF00]">Approve</button>
                            )}
                          </div>
                        </div>
                        {idea.trending_angle && (
                    ...
                    {pipelineItems.filter(p => p.stage === stage && (pipelineFilter === "all" || p.type === pipelineFilter)).map(item => (
                      <div key={item.id} className="glass rounded-xl p-3 text-xs">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-text-muted">
                          <span className={`px-1.5 py-0.5 rounded ${item.type === "blog" ? "bg-info/10 text-info" : item.type === "video" ? "bg-danger/10 text-danger" : item.type === "email" ? "bg-purple-500/10 text-purple-700" : "bg-[rgba(212, 255, 0,0.08)] text-brand-accent"}`}>{item.type}</span>
            ...
            <div className="space-y-2">
              {remixOptions.alternatives.map((alt, i) => (
                <button
                  ...
                  className="w-full text-left p-3 border border-[rgba(212, 255, 0,0.25)] rounded-lg hover:border-brand-accent hover:bg-[rgba(212, 255, 0,0.05)] transition-all"
                >
