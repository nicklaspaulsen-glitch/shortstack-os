...
const COLOR_PALETTES = [
  { name: "Professional", colors: ["#1a1a2e", "#16213e", "#0f3460", "#e94560", "#D4FF00"] },
  { name: "Fresh & Clean", colors: ["#ffffff", "#f0f0f0", "#2d3436", "#00cec9", "#6c5ce7"] },
  { name: "Bold & Vibrant", colors: ["#D4FF00", "#feca57", "#48dbfb", "#ff9ff3", "#54a0ff"] },
  { name: "Luxury Gold", colors: ["#0d0d0d", "#1a1a1a", "#D4FF00", "#e8d5a3", "#ffffff"] },
  ...
  { name: "Minimal Dark", colors: ["#0a0a0a", "#1a1a1a", "#333333", "#ffffff", "#D4FF00"] },
...
const PHOTO_FILTERS = [
  ...
  { name: "Retro", css: "sepia(0.5) hue-rotate(-10deg) saturate(1.5)" },
  ...
  { name: "Ocean Blue", css: "hue-rotate(30deg) saturate(1.5) brightness(1.05)" },
  ...
  { name: "Bloom", css: "brightness(1.15) contrast(0.95) saturate(1.5) blur(0.3px)" },
];
...
export default function DesignStudioPage() {
  ...
  const [brandKits, setBrandKits] = useState<BrandKit[]>([
    { id: "1", name: "Default Brand", colors: ["#D4FF00", "#1a1a2e", "#16213e", "#ffffff", "#f0f0f0"], fonts: { heading: "Montserrat Bold", body: "Open Sans Regular" }, logoUrl: "" },
  ]);
  const [activeBrandKit, setActiveBrandKit] = useState<string>("1");
  const [newBrandKitName, setNewBrandKitName] = useState("");
  const [newBrandColor, setNewBrandColor] = useState("#D4FF00");
  ...
  const [patternColor1, setPatternColor1] = useState("#D4FF00");
  ...
  const [newMoodColor, setNewMoodColor] = useState("#D4FF00");
  ...
  function handleExtractColors() {
    ...
    setTimeout(() => {
      setExtractedColors(["#2d3436", "#636e72", "#b2bec3", "#dfe6e9", "#D4FF00", "#e17055"]);
    ...
    },
    {
      ...
      component: (
        <textarea
          ...
          rows={3}
          className="w-full px-4 py-3 rounded-xl bg-surface-light border border-border-subtle text-sm focus:outline-none focus:border-brand-accent/50 focus:ring-2 focus:ring-[rgba(212, 255, 0,0.12)] transition-all resize-none"
          autoFocus
        />
      ),
    },
        ...
        <>
            ...
            {tab === "create" && (
              <div className="space-y-4">
                ...
                {selectedTemplate && (
                  <div className="flex items-center gap-3 bg-[rgba(212, 255, 0,0.03)] border border-[rgba(212, 255, 0,0.15)] rounded-xl px-4 py-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  <button onClick={() => setActiveSection(null)}
                    className={`text-[10px] px-3 py-1.5 rounded-lg border transition-all ${!activeSection ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent font-medium" : "border-border-subtle text-text-muted hover:text-text-primary"}`}>
                    All
                  </button>
                  {SECTIONS.map(s => (
                    <button key={s.key} onClick={() => setActiveSection(s.key)}
                      className={`text-[10px] px-3 py-1.5 rounded-lg border transition-all flex items-center gap-1.5 ${
                        activeSection === s.key ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent font-medium" : "border-border-subtle text-text-muted hover:text-text-primary"
                      }`}>
                      {s.icon} {s.label}
                    </button>
                  ))}
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-2">
                                <span className="badge bg-[rgba(212, 255, 0,0.08)] text-brand-accent text-[10px] px-2 py-0.5 rounded capitalize">
                                  {g.section}
                                </span>
            ...
            {tab === "tools" && (
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button key={tool.id} onClick={() => setToolsTab(tool.id)}
                      className={`text-[10px] px-3 py-1.5 rounded-lg border transition-all flex items-center gap-1.5 ${
                        toolsTab === tool.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent font-medium" : "border-border-subtle text-text-muted hover:text-text-primary"
                      }`}>
                ...
                {toolsTab === "brand-kit" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {brandKits.map(kit => (
                        <div key={kit.id} className={`card-hover rounded-xl cursor-pointer transition-all ${activeBrandKit === kit.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.03)]" : ""}`}
                          ...
                          <div className="flex items-center justify-between mb-2">
                            ...
                            {activeBrandKit === kit.id && <span className="text-[9px] text-brand-accent font-medium bg-[rgba(212, 255, 0,0.08)] px-2 py-0.5 rounded">Active</span>}
                          </div>
                          <div className="flex gap-1 mb-2">
                            {kit.colors.map((c, i) => (
                        ...
                        {RESIZE_PRESETS.map(preset => {
                          ...
                          return (
                            <button key={key} onClick={() => setSelectedResizes(prev => isSelected ? prev.filter(r => r !== key) : [...prev, key])}
                              className={`card-hover rounded-xl p-2.5 text-left transition-all ${isSelected ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)]" : ""}`}>
                        ...
                        <div key={layer.id} className="flex items-center gap-2 p-2.5 rounded-xl border border-border-subtle hover:border-[rgba(212, 255, 0,0.2)] transition-all bg-surface-light/30">
                          ...
                          <div className="flex items-center gap-1.5">
                            <input type="range" min={0} max={100} value={layer.opacity}
                              ...
                              className="w-14 h-1 accent-[#D4FF00]" />
                ...
                {toolsTab === "bg-remover" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><Scissors size={16} /></div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {MOCKUP_PRODUCTS.map(product => (
                        <button key={product.id} onClick={() => setSelectedMockup(product.id)}
                          className={`card-hover rounded-xl p-3 text-center transition-all ${selectedMockup === product.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)]" : ""}`}>
                ...
                {toolsTab === "qr-code" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><QrCode size={16} /></div>
                      <div>
                      ...
                      <div className="flex items-end">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={qrIncludeLogo} onChange={e => setQrIncludeLogo(e.target.checked)}
                            className="w-3.5 h-3.5 rounded accent-[#D4FF00]" />
                      ...
                      <div className="space-y-2">
                        ...
                        {versionHistory.map((v, i) => (
                          <div key={v.id} className={`flex items-start gap-3 p-2.5 rounded-xl border cursor-pointer transition-all ${
                            selectedVersion === v.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.03)]" : "border-border-subtle hover:border-[rgba(212, 255, 0,0.15)]"
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {i === 0 && <span className="text-[8px] bg-[rgba(212, 255, 0,0.08)] text-brand-accent px-1.5 py-0.5 rounded">Latest</span>}
                              </div>
                        ...
                        <div className="flex gap-2">
                          {extractedColors.map((c, i) => (
                            <div key={i} className="flex-1 text-center cursor-pointer group" onClick={() => { navigator.clipboard.writeText(c); toast.success(`Copied ${c}`); }}>
                              <div className="h-14 rounded-xl border border-border-subtle group-hover:ring-2 ring-[rgba(212, 255, 0,0.2)] transition-all" style={{ background: c }} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {FONT_PAIRINGS.map((pair, i) => (
                        <button key={i} onClick={() => { setSelectedPairing(i); toast.success(`Selected: ${pair.vibe}`); }}
                          className={`card-hover rounded-xl p-3 text-left transition-all ${selectedPairing === i ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.03)]" : ""}`}>
                          <div className="flex items-center justify-between mb-2">
                ...
                {toolsTab === "patterns" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><Repeat size={16} /></div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {PATTERN_TYPES.map(p => (
                        <button key={p.id} onClick={() => setPatternType(p.id)}
                          className={`card-hover rounded-xl p-2.5 text-left transition-all ${patternType === p.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)]" : ""}`}>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <input type="range" min={10} max={100} value={patternScale} onChange={e => setPatternScale(parseInt(e.target.value))}
                          className="w-full h-1.5 accent-[#D4FF00] mt-2" />
                      </div>
                    </div>
                ...
                {toolsTab === "filters" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><Aperture size={16} /></div>
                      <div>
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <input type="range" min={0} max={100} value={filterIntensity} onChange={e => setFilterIntensity(parseInt(e.target.value))}
                        className="flex-1 h-1.5 accent-[#D4FF00]" />
                    </div>

                    <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                      {PHOTO_FILTERS.map(filter => (
                        <button key={filter.name} onClick={() => { setActiveFilter(filter.name); toast.success(`Filter: ${filter.name}`); }}
                          className={`rounded-xl overflow-hidden border transition-all ${activeFilter === filter.name ? "border-brand-accent/40 ring-2 ring-[rgba(212, 255, 0,0.12)]" : "border-border-subtle hover:border-[rgba(212, 255, 0,0.2)]"}`}>
                          <div className="h-16 bg-gradient-to-br from-[rgba(212, 255, 0,0.12)] via-blue-500/20 to-purple-500/20"
                    ...
                    {activeFilter && (
                      <div className="flex items-center gap-2 bg-[rgba(212, 255, 0,0.03)] border border-[rgba(212, 255, 0,0.15)] rounded-xl px-3 py-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {ICON_CATEGORIES.map(cat => (
                        <button key={cat.name} onClick={() => setIconCategory(cat.name)}
                          className={`text-[9px] px-2.5 py-1 rounded-lg border transition-all ${iconCategory === cat.name ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent font-medium" : "border-border-subtle text-text-muted"}`}>
                          {cat.name}
                        </button>
                      ))}
                    </div>
                      ...
                      {(ICON_CATEGORIES.find(c => c.name === iconCategory)?.icons || [])
                        ...
                        .map(icon => (
                          <button key={icon} onClick={() => { navigator.clipboard.writeText(icon); toast.success(`Copied icon: ${icon}`); }}
                            className="card-hover rounded-xl p-3 text-center group hover:bg-[rgba(212, 255, 0,0.05)]">
                            <div className="w-6 h-6 mx-auto mb-1 rounded-lg bg-surface-light flex items-center justify-center group-hover:bg-[rgba(212, 255, 0,0.08)]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {INFOGRAPHIC_TYPES.map(type => (
                        <button key={type.id} onClick={() => setInfographicType(type.id)}
                          className={`card-hover rounded-xl p-3 text-left transition-all ${infographicType === type.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)]" : ""}`}>
                ...
                {toolsTab === "social-proof" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><ThumbsUp size={16} /></div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {SOCIAL_PROOF_TYPES.map(type => (
                        <button key={type.id} onClick={() => setSocialProofType(type.id)}
                          className={`card-hover rounded-xl p-3 text-left transition-all ${socialProofType === type.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)]" : ""}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {SEASONAL_PACKS.map(pack => (
                        <button key={pack.id} onClick={() => { setSelectedSeason(pack.id); toast.success(`Selected: ${pack.label}`); }}
                          className={`card-hover rounded-xl p-3 text-left transition-all ${selectedSeason === pack.id ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)]" : ""}`}>
                          <div className="flex items-center gap-2 mb-2">
                ...
                {toolsTab === "animation" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {(["fade", "slide", "bounce", "zoom", "rotate"] as const).map(type => (
                        <button key={type} onClick={() => setAnimationType(type)}
                          className={`card-hover rounded-xl p-2.5 text-center transition-all ${animationType === type ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)]" : ""}`}>
                    ...
                    <div className="bg-surface-light rounded-xl border border-border-subtle p-8 flex items-center justify-center min-h-[200px]">
                      <div className={`w-32 h-32 bg-[rgba(212, 255, 0,0.12)] rounded-xl flex items-center justify-center border border-[rgba(212, 255, 0,0.25)] transition-all ${
                        animationPlaying ? (
                ...
                {toolsTab === "design-tokens" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><Database size={16} /></div>
                      <div>
                        ...
                        {["png", "jpg", "webp", "svg", "pdf"].map(fmt => (
                          <button key={fmt} onClick={() => setBatchExportFormats(prev => prev.includes(fmt) ? prev.filter(f => f !== fmt) : [...prev, fmt])}
                            className={`text-[10px] px-3 py-1.5 rounded-lg border uppercase font-medium transition-all ${
                              batchExportFormats.includes(fmt) ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent" : "border-border-subtle text-text-muted"
                            }`}>
                            .{fmt}
                          </button>
                        ))}
                        ...
                        {[
                          ...
                          <button key={s.size} onClick={() => setBatchExportSizes(prev => prev.includes(s.size) ? prev.filter(x => x !== s.size) : [...prev, s.size])}
                            className={`card-hover rounded-xl p-2 text-center text-[10px] transition-all ${
                              batchExportSizes.includes(s.size) ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent font-medium" : ""
                            }`}>
                  ...
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    ...
                    <div>
                      ...
                      <input type="range" min={8} max={72} value={a11yFontSize} onChange={e => setA11yFontSize(parseInt(e.target.value))}
                        className="w-full h-1.5 accent-[#D4FF00]" />
                    </div>
                ...
                {toolsTab === "mood-board" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><Heart size={16} /></div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {moodBoardItems.map(item => (
                        <div key={item.id} className="card-hover rounded-xl p-2.5 relative group">
                          ...
                          {item.type === "note" && (
                            <div className="bg-[rgba(212, 255, 0,0.03)] border border-[rgba(212, 255, 0,0.15)] rounded-lg p-2 min-h-[64px]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div className="border border-dashed border-border-subtle rounded-xl p-3 flex items-center justify-center cursor-pointer hover:border-[rgba(212, 255, 0,0.25)] transition-all"
                        onClick={() => {
                ...
                {toolsTab === "style-transfer" && (
                  <motion.div className="glass rounded-xl space-y-4 p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-9 h-9 bg-[rgba(212, 255, 0,0.08)] rounded-xl flex items-center justify-center text-brand-accent"><Shuffle size={16} /></div>
                      <div>
                        ...
                        {["Oil Painting", "Watercolor", "Pixel Art", "Comic Book", "Art Deco", "Impressionist", "Cyberpunk", "Minimalist"].map(preset => (
                          <button key={preset} onClick={() => { setStyleTransferStyle(preset); toast.success(`Style: ${preset}`); }}
                            className={`text-[9px] px-2.5 py-1.5 rounded-lg border transition-all ${
                              styleTransferStyle === preset ? "border-[rgba(212, 255, 0,0.25)] bg-[rgba(212, 255, 0,0.05)] text-brand-accent font-medium" : "border-border-subtle text-text-muted hover:text-text-primary"
                            }`}>
                            {preset}
                          </button>
                        ))}
