...
export default function ServicesPage() {
  ...
  return (
    <MotionPage className="space-y-6">{/* -- Service Catalog command strip -- */}
    <div className="flex items-center justify-between gap-4 px-1 py-3 sm:py-4">
      <div className="min-w-0">
        <p className="font-editorial text-[11px] italic text-text-muted mb-0.5">Service Catalog</p>
        <h1 className="font-display text-xl sm:text-2xl font-bold tracking-tight text-text-primary leading-none">Service Catalog</h1>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <button onClick={() => setShowCreate((v) => !v)}
              ...
              <div className="glass rounded-xl p-5">
                <p className="font-semibold text-text-secondary text-sm mb-4">New Service</p>
              ...
              <div className="glass rounded-xl p-12 flex flex-col items-center gap-4 text-center">
                ...
                <p className="text-text-secondary font-semibold">No services yet</p>
              ...
              <div className="glass rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-white/8">
                    {services.map((s, idx) =>
                      editId === s.id ? (
                        <tr key={s.id} className="bg-white/5">
                        ...
                        <motion.tr key={s.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.04 }} className={`hover:bg-[rgba(0,0,0,0.02)] transition-colors group ${!s.is_active ? "opacity-50" : ""}`}>
                          ...
                          <td className="px-4 py-3 text-right text-text-secondary font-medium hidden sm:table-cell">
                            {formatPrice(s.price_cents, s.billing_interval)}
                          </td>
                          <td className="px-4 py-3 text-center hidden md:table-cell">
                            <button onClick={() => handleToggleActive(s)}
                              className={`text-xs px-2 py-0.5 rounded-full border ${s.is_active ? "border-green-500/40 text-green-400 bg-green-500/10" : "border-border-subtle text-text-muted"}`}>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                              <button onClick={() => { setEditId(s.id); setEditForm(serviceToForm(s)); }}
                                className="p-1.5 rounded hover:bg-white/8 text-text-muted hover:text-text-secondary" title="Edit">
                                <Pencil size={13} />
                              </button>
