...
function PresetsTab({ presets, loading, onRefresh }: { presets: VoiceClone[]; loading?: boolean; onRefresh?: () => void }) {
  ...
  const [detailPreset, setDetailPreset] = useState<VoiceClone | null>(null);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Voice detail side-panel */}
      {detailPreset && (
        <PresetEditExamplePanel
          kind="voice"
          preset={{
            id: detailPreset.id,
            name: detailPreset.label,
            category: (detailPreset.consent_evidence?.category as string) || "preset",
            language: detailPreset.language?.toUpperCase() || "EN",
            gender: inferGender(detailPreset),
            description: detailPreset.description ?? undefined,
            useCases: presetUseCases(detailPreset),
            defaultExampleText: textCache[detailPreset.id] ?? presetDefaultText(detailPreset),
            suggestedScripts: [...presetSuggestedScripts(detailPreset)],
            previewUrl: previewCache[detailPreset.id] ?? undefined,
          }}
...
interface PresetCardProps {
  ...
  /** Called when the user opens the detail panel for this preset. */
  onDetails?: () => void;
}

function PresetCard({ preset, cachedUrl, cachedText, onUrlCached, onTextChanged, onSaved, featured, favorited, onToggleFavorite, onDetails }: PresetCardProps) {
  const [testing, setTesting] = useState(false);
