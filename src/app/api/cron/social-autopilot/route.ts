...
interface SocialAutopilotConfig {
  ...
  auto_hashtag_research: boolean;
  posts_per_day: number;
  content_mix: { educational: number; entertaining: number; promotional: number; social_proof: number };
...
export async function GET(request: NextRequest) {
  ...
  for (const client of clients || []) {
    ...
    // Auto-generate content when queue is low (< 3 days ahead)
...
Return ONLY the JSON array.`,
  ...
  if (results.content_generated > 0 || results.posts_published > 0) {
    await supabase.from("trinity_log").insert({
      ...
      description: `[CRON] Social Autopilot: ${results.content_generated} generated, ${results.posts_published} published`,
      status: "completed",
      metadata: results,
    });
  }
