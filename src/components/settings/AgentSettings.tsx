...
export default function AgentSettings({ agentConfigs, editingAgent, setEditingAgent, saveAgentConfig }: Props) {
  return (
    <div className="space-y-4">
      <div className="glass rounded-xl p-4 bg-[rgba(59,130,246,0.05)] border-[rgba(59,130,246,0.2)]">
          ...
          <div key={config.client_id} className="card-hover">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between">
                <span className="text-text-muted">DM Outreach</span>
                <StatusBadge status={config.outreach_enabled ? "active" : "paused"} />
              </div>
              <div className="flex justify-between">
                <span className="text-text-muted">Cold Calling</span>
                <StatusBadge status={config.cold_calling_enabled ? "active" : "paused"} />
              </div>
              <div className="flex justify-between">
                <span className="text-text-muted">Content Gen</span>
      ...
      <div className="glass rounded-xl p-4 mt-6">
        <h3 className="">Global AI Settings</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-xs text-text-muted mb-1">Default AI Model</label>
          ...
          <div>
            <label className="block text-xs text-text-muted mb-1">Daily DM Target</label>
            <input type="number" defaultValue={80} className="input w-full text-sm" />
          </div>
          <div>
            <label className="block text-xs text-text-muted mb-1">Daily Call Target</label>
            <input type="number" defaultValue={50} className="input w-full text-sm" />
          </div>
          <div>
            <label className="block text-xs text-text-muted mb-1">Outreach Time (CET)</label>
        ...
        {editingAgent && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-text-muted mb-1">AI Model</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-text-muted mb-1">Daily DM Limit</label>
              ...
              <div>
                <label className="block text-sm text-text-muted mb-1">Daily Call Limit</label>
            ...
            <div>
              <label className="block text-sm text-text-muted mb-1">Outreach Platforms</label>
            ...
            <div>
              <label className="block text-sm text-text-muted mb-1">Custom Instructions for AI</label>
