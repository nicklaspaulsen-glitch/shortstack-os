  ...
  if (!providers) {
    return (
      <div className=" border border-border-subtle bg-surface p-4">
        <h2 className="text-xs font-bold mb-3 text-text-primary">Webhook dedup health</h2>
  ...
  return (
    <div className=" border border-border-subtle bg-surface p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-xs font-bold text-text-primary">Webhook dedup health</h2>
        <span
      ...
      {providers.length === 0 ? (
        <p className="text-[11px] text-text-muted">No processed_events rows found.</p>
      ) : (
        <table className="w-full text-[11px]">
          ...
          <tbody>
            {providers.map((p) => (
              <tr key={p.provider} className="border-t border-border-subtle/50">
                <td className="py-1.5 font-mono text-text-primary">{p.provider}</td>
                <td className="py-1.5 text-right text-text-muted">{p.done}</td>
                <td
                  className={`py-1.5 text-right font-medium ${
                    p.stuck ? "text-danger" : "text-text-primary"
                  }`}
                >
                  {p.processing}
                </td>
                <td className="py-1.5 text-right text-text-muted">{p.in_flight}</td>
