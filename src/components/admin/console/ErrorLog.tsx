    ...
    <div className="border-t border-border-subtle/50">
      ...
      {open && (
        <div className="px-4 pb-3 space-y-2">
          ...
          <div>
            <div className="text-[10px] text-text-muted mb-1">result</div>
            <pre className="bg-background rounded p-2 text-[10px] font-mono text-text-primary overflow-x-auto max-h-32">
              {safeJson(row.result)}
            </pre>
          </div>
          <div>
            <div className="text-[10px] text-text-muted mb-1">metadata</div>
            <pre className="bg-background rounded p-2 text-[10px] font-mono text-text-primary overflow-x-auto max-h-32">
...
export default function ErrorLog({ rows }: ErrorLogProps) {
  if (!rows) {
    return (
      <div className=" border border-border-subtle bg-surface p-4">
        <h2 className="text-xs font-bold mb-3 text-text-primary">Recent error log</h2>
  ...
  return (
    <div className=" border border-border-subtle bg-surface p-4">
      ...
      {rows.length === 0 ? (
        <p className="text-[11px] text-text-muted">No errors logged.</p>
      ) : (
        <div>
          {rows.map((row) => (
