    ...
    <div className="glass rounded-xl p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Shield size={14} className="text-[#D4FF00]" />
          <h3 className="text-sm font-semibold">Outreach Accounts</h3>
          <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[rgba(212,255,0,0.08)] text-[#D4FF00] border border-[rgba(212,255,0,0.2)]">
                  ...
                  <div key={sender.id} className={`group flex items-center gap-2 p-2 rounded-lg border transition-all ${
                    ...
                    <button
                      ...
                      className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-all ${
                        sender.is_active !== false ? "bg-[#D4FF00] border-[#D4FF00]" : "border-border-subtle"
                      }`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {sender.is_primary && (
                          <span className="text-[8px] px-1 py-0.5 rounded bg-[rgba(212,255,0,0.08)] text-[#D4FF00]">Primary</span>
                        )}
                      </div>
                    ...
                    {!sender.is_primary && sender.is_active !== false && (
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-[9px] text-text-muted hover:text-[#D4FF00] transition-opacity"
                      >
                        Set primary
                      </button>
                    )}
                ...
                return (
                  <div key={phone.id} className={`flex items-center gap-2 p-2 rounded-lg border transition-all ${
                    ...
                    <button
                      ...
                      className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-all ${
                        phone.is_active !== false ? "bg-[#D4FF00] border-[#D4FF00]" : "border-border-subtle"
                      }`}
                    >
              ...
              {socials.length === 0 ? (
                ...
                <div key={acc.id} className={`flex items-center gap-2 p-2 rounded-lg border transition-all ${
                  ...
                  <button
                    ...
                    className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-all ${
                      acc.is_active !== false ? "bg-[#D4FF00] border-[#D4FF00]" : "border-border-subtle"
                    }`}
                  >
      ...
      <div className="mt-3 pt-3 border-t border-border-subtle flex items-center justify-between text-[10px] text-text-muted">
        ...
        {activeTab === "social" ? (
          ...
          <Link
            ...
            className="flex items-center gap-1 text-[#D4FF00] hover:text-[#E8FF4D] transition-colors"
          >
...
function EmptyState({ icon, label, linkText, link }: { icon: React.ReactNode; label: string; linkText: string; link: string }) {
  return (
    <div className="py-6 text-center">
      ...
      <Link href={link} className="inline-flex items-center gap-1 text-[10px] text-[#D4FF00] hover:underline">
