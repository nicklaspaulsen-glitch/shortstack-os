    ...
    <>
      <button
        ...
        className={`group relative w-full md:w-auto  overflow-hidden transition-all hover-lift ${
          hasCached
            ? "bg-gradient-to-r from-[#D4FF00] via-[#E8FF4D] to-[#D4FF00] shadow-lg shadow-[rgba(212,255,0,0.4)]"
            : "bg-gradient-to-r from-[#D4FF00] to-[#AACC00] shadow-lg shadow-[rgba(212,255,0,0.3)]"
        }`}
      >
        {/* Glow pulse */}
      ...
      {open && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="relative px-6 py-4 border-b border-border-subtle bg-gradient-to-br from-[rgba(212,255,0,0.08)] via-transparent to-[rgba(212,255,0,0.04)] overflow-hidden">
              <div className="absolute inset-0 opacity-30">
                <div className="absolute top-0 left-1/4 w-32 h-32 rounded-full bg-[rgba(212,255,0,0.18)] blur-3xl" />
            ...
            {/* Schedule panel */}
            {showSchedule && (
              <div className="px-6 py-4 border-t border-border-subtle bg-[rgba(212,255,0,0.03)]">
                <div className="flex items-start gap-3 mb-3">
                  <Clock size={14} className="text-[#D4FF00] shrink-0 mt-0.5" />
                  <div className="flex-1">
...
function RecommendationCard({ rec, index }: { rec: Recommendation; index: number }) {
  ...
  const effortColor = rec.effort === "quick" ? "text-indigo-400" : rec.effort === "medium" ? "text-purple-400" : "text-pink-400";

  return (
    <div
      className="relative group rounded-xl border border-border-subtle bg-surface-light/30 hover:border-[rgba(212,255,0,0.25)] hover:bg-surface-light/60 transition-all overflow-hidden"
      style={{ animationDelay: `${index * 40}ms` }}
    >
      <div className="p-4 space-y-2.5">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div className="w-8 h-8 rounded-lg bg-[rgba(212,255,0,0.08)] text-[#D4FF00] flex items-center justify-center shrink-0">
              {icon}
            </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border-subtle/50">
          ...
          <Link
            ...
            className="flex items-center gap-1 text-[11px] font-semibold text-[#D4FF00] hover:text-[#E8FF4D] transition-colors"
          >
