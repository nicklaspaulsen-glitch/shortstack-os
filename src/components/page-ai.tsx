      ...
      >
        ...
        <style jsx>{`
          ...
          .orb-halo {
            ...
            background: radial-gradient(circle, rgba(212,255,0,0.5) 0%, transparent 70%);
            ...
            <div className="space-y-1">
              {suggestions.map((s, i) => (
                <button key={i} onClick={() => sendMessage(s)}
                  className="w-full text-left text-[10px] p-2 rounded-lg border border-border-subtle/20 hover:border-[rgba(212,255,0,0.2)] text-text-muted hover:text-text-primary transition-all">
                  {s}
                </button>
              ))}
            </div>
          ...
          return (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] rounded-xl px-3 py-2 ${
                msg.role === "user"
                  ? "bg-[rgba(212,255,0,0.08)] border border-[rgba(212,255,0,0.08)]"
