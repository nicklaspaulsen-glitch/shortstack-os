...
export default function OfficeEventFeed({ events, onSelectAgent }: FeedProps) {
  return (
    <div className="flex h-full flex-col overflow-hidden  border border-border-subtle bg-[#0a0a0f]/85 backdrop-blur-md">
              ...
              <li key={`${ev.agentKey}-${ev.ts}-${idx}`}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 line-clamp-2 text-xs leading-snug text-text-primary">
                      {ev.summary}
                    </div>
                  </div>
                </button>
...
export function OnlineNow({ rows }: OnlineNowProps) {
  return (
    <div className="flex items-center gap-2  border border-border-subtle bg-[#0a0a0f]/85 px-4 py-3 backdrop-blur-md">
      ...
      <div className="ml-auto flex -space-x-1.5">
        {rows.slice(0, 4).map((r, i) => (
          <span
            ...
            className="h-5 w-5 rounded-full border border-border-subtle bg-gradient-to-br from-lime-400 to-emerald-600"
            title="Online"
          />
        ))}
      </div>
