    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[11px] font-bold text-[#D4FF00]">{Math.round(progress)}%</span>
        </div>
        <div className="h-1.5 rounded-full bg-surface-light border border-border-subtle overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-[#D4FF00] to-[#AACC00] transition-all duration-500"
            ...
            <button
              ...
              className={`rounded-full transition-all duration-200 ${
                i === stepIdx
                  ? "w-6 h-1.5 bg-[#D4FF00]"
                  : i < stepIdx
                  ? "w-1.5 h-1.5 bg-[rgba(212,255,0,0.4)] hover:bg-[rgba(212,255,0,0.7)] cursor-pointer"
        ...
        {stepIdx < STEPS.length - 1 ? (
          <div className="flex items-center gap-2">
            {stepKey === "personalize" && (
              <button
                onClick={next}
                className="text-[11px] text-text-muted hover:text-[#D4FF00] transition-colors px-3 py-2 font-medium"
              >
                Skip for now
              </button>
            )}
            <button
              onClick={next}
              disabled={!canAdvance()}
              className="flex items-center gap-1.5 px-5 py-2 rounded-lg bg-gradient-to-r from-[#D4FF00] to-[#AACC00] text-white text-xs font-bold hover:shadow-lg hover:shadow-[rgba(212,255,0,0.2)] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
          ...
          return (
            <button
              ...
              className={`relative text-left p-4  border transition-all hover-lift ${
                isSelected
                  ? "border-[#D4FF00] bg-[rgba(212,255,0,0.08)] shadow-[0_0_0_2px_rgba(212,255,0,0.18)]"
                  : "border-border-subtle bg-surface-light hover:border-[rgba(212,255,0,0.25)]"
              }`}
            >
              <div className="flex items-center gap-2 mb-2">
                <div
                  className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                    isSelected ? "bg-[rgba(212,255,0,0.12)] text-[#D4FF00]" : "bg-surface-light text-text-muted"
                  }`}
                >
                  <Icon size={18} />
                </div>
                {isSelected && (
                  <span className="ml-auto w-5 h-5 rounded-full bg-[#D4FF00] flex items-center justify-center">
...
function StepBusinessInfo({
  ...
  return (
    <div>
      <div className="mb-6">
        <p className="text-[11px] uppercase tracking-widest text-[#D4FF00] font-semibold mb-1">
          {userTypeLabel}
        </p>
      ...
      <div className="space-y-5">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2.5 rounded-xl bg-surface-light border border-border-subtle text-sm focus:outline-none focus:border-[#D4FF00] focus:ring-2 focus:ring-[rgba(212,255,0,0.2)] transition-all"
          />
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl bg-surface-light border border-border-subtle text-sm focus:outline-none focus:border-[#D4FF00] focus:ring-2 focus:ring-[rgba(212,255,0,0.2)] transition-all"
            />
          </div>
        </div>
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h2 className="text-2xl font-bold mb-1 flex items-center gap-2">
          <Target size={22} className="text-[#D4FF00]" /> Your niche / focus
        </h2>
    ...
    <div>
      <div className="mb-6 flex items-start justify-between gap-3">
        ...
        <button
          ...
          disabled={loading}
          className="text-[10px] text-text-muted hover:text-[#D4FF00] transition-colors whitespace-nowrap disabled:opacity-40"
        >
          Regenerate
        </button>
      </div>
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-[11px] border transition-all ${
                  sel
                    ? "bg-[rgba(212,255,0,0.08)] border-[rgba(212,255,0,0.4)] text-[#D4FF00]"
    ...
    return (
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-4 py-2.5 rounded-xl bg-surface-light border border-border-subtle text-sm focus:outline-none focus:border-[#D4FF00] focus:ring-2 focus:ring-[rgba(212,255,0,0.2)] transition-all resize-none"
        />
      </div>
    );
  }
          ...
          {questions.map((q) => (
            <div key={q.id} className="rounded-xl border border-border-subtle p-4 bg-surface-light/30">
              ...
              <textarea
                ...
                placeholder={q.placeholder}
                rows={2}
                className="w-full px-3 py-2.5 bg-surface-light border border-border-subtle rounded-lg text-sm text-text-primary focus:outline-none focus:border-[#D4FF00] focus:ring-2 focus:ring-[rgba(212,255,0,0.2)] transition-all resize-none"
              />
            </div>
          ))}
  ...
  return (
    <div className="text-center py-6">
      ...
      <p className="text-sm text-text-muted max-w-md mx-auto mb-6">
        ...
        dashboard will focus on <span className="text-[#D4FF00] font-medium">{userTypeLabel}</span> metrics and your
        sidebar has <span className="text-[#D4FF00] font-medium">{enabledCount}</span> tools enabled.
      </p>
      <div className="grid grid-cols-3 gap-3 max-w-md mx-auto">
        {[
          ...
          <div key={x.label} className="p-3 rounded-xl border border-border-subtle bg-surface-light/40">
            <div className="text-[#D4FF00] mb-1 inline-flex">{x.icon}</div>
