...
export const ROUTES_TO_CHECK: SelfTestCheck[] = [
  ...
  // ── Outreach quality (email/phone validation + news triggers) ────────────
  {
    path: "/api/integrations/validate-email",
    method: "POST",
    auth_bearer: true,
    body: { emails: [SELF_TEST_SENTINEL_EMAIL] },
    expected_status: [200, 401],
    note: "Validate-email — soft-fails to 'skipped' result when no key set; 200 with results map expected when authed.",
    timeout_ms: 25_000,
  },
  {
    path: "/api/integrations/validate-email",
    ...
    body: {},
    expected_status: [400, 401],
    note: "Validate-email empty body — must 400 (emails array required), not 500.",
  },
  {
    path: "/api/integrations/validate-phone",
    method: "POST",
    auth_bearer: true,
    body: { phones: [SELF_TEST_SENTINEL_PHONE] },
    expected_status: [200, 401],
    note: "Validate-phone — soft-fails to 'skipped' when no key set; uses Twilio magic number for safety.",
    timeout_ms: 25_000,
  },
  {
    path: "/api/integrations/news-triggers",
    auth_bearer: true,
    expected_status: [200, 401],
    note: "News triggers list — 200 with empty array when no hits yet.",
  },
  {
    path: `/api/integrations/news-triggers/${SELF_TEST_DUMMY_JOB_ID}/acknowledge`,
    method: "POST",
    auth_bearer: true,
    expected_status: [401, 404],
    note: "Acknowledge with dummy id — 404 (not found) or 401 (unauthed) expected.",
  },
  {
    path: "/api/cron/news-triggers-scan",
    expected_status: [200, 401],
    note: "News-triggers daily cron — 401 without bearer; 200 when bearer matches CRON_SECRET.",
    timeout_ms: 25_000,
  },
