...
async function rasterize(svgBuffer: Buffer, size: number): Promise<Buffer> {
  // density = 72 * (size/64) ensures sharp uses a resolution that the internal
  // librsvg renders crisply at the target size. Without it, tiny sizes (16px)
  // get aliased and large sizes (1024px) get pixellated.
  const density = Math.max(72, Math.round((size / 64) * 288));
