...
--
-- Three tables: case_studies, case_study_assets, case_study_views.
...
-- Public: anyone can read published case studies.
...
-- Org members (owner or team_member via parent_agency) can read their own (incl. drafts).
...
-- Org members can update own org's.
...
-- Only admins (agency owners) can delete.
...
-- Public read when parent is published.
...
-- Org members read all assets of own org.
...
-- Org members can write (insert/update/delete) assets for own org.
...
-- Anyone (anonymous fetch included) can insert a view record. The route
-- gates by published-status server-side before logging.
...
-- Org members read view rows for their own case studies.
